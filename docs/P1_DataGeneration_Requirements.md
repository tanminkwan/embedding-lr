...
- v0.1 데이터(`data/v0.1_from.Claude-Cowork/`, CSV 포맷)가 이미 생성되어 있으나,
  ...
  1. `role_03_network.csv`의 CSV 이스케이프 오류로 레코드 1건 손실 + 파싱 오류 행 6건 발생
     ...
     문제. v0.2부터는 이 이스케이프 규칙 자체가 필요 없는 **JSONL**로 출력 포맷을
...
- 본 문서는 이 두 문제를 해결한 **v0.2 재생성**(JSONL 포맷 전환 포함)을 포함하여, Phase 1의
  요구사항을 명문화한다.
...
| 3 | `role_01~09_*.jsonl` (역할별 원본, source of truth) 생성 |
| 4 | `role_01~09_*.jsonl` → `data.jsonl` 재조합 (concat) |
| 5 | `data.jsonl` → `train.jsonl`/`test.jsonl`/`val.jsonl` 클래스별 3:1:1 **시드 고정** 분할 |
| 6 | JSONL 무결성 준수 — 매 줄이 정확히 레코드 1건에 대응하는 유효한 JSON 객체(응답 내 개행은 JSON 문자열 이스케이프 `\n`으로 표현되어 줄 경계를 깨지 않아야 함) |
...
각 줄은 아래 3개 키를 가진 JSON 객체 1건이다.

| 키 | 타입 | 허용값 |
|---|---|---|
| `질의` | string | 2~100자 (기존 데이터 실측 2~63자 참고) |
| `응답` | string | 8~1000자 (기존 데이터 실측 8~671자 참고), 멀티라인인 경우 JSON 문자열 이스케이프(`\n`)로 표현 |
...
| 인코딩 | UTF-8, BOM 없음 |
| 개행(파일 구분자) | 레코드 구분은 LF만 허용(CRLF 금지) — 파일 내 물리적 줄 수 = 레코드 수와 항상 일치해야 함 |
| JSON 유효성 | 매 줄이 표준 `json` 라이브러리로 파싱 가능한 객체 1건이어야 함 — `role_03_network.csv` 사고(3.1절, CSV 이스케이프 오류)와 같은 유형의 파싱 실패 원천 차단(JSON 표준 이스케이프는 언어 표준 라이브러리가 처리하므로 수기 이스케이프 규칙 불필요) |
...
| 파일 | 설명 | 명명 규칙 근거 |
|---|---|---|
| `data/<version>/role_01~09_*.jsonl` | 역할/카테고리별 원본 (source of truth) | v0.2부터 JSONL로 전환 |
| `data/<version>/data.jsonl` | 9개 role 파일 재조합본 (1,000건) | v0.2부터 JSONL로 전환 |
| `data/<version>/train.jsonl` | 학습셋 (600건) | v0.2부터 JSONL로 전환 |
| `data/<version>/test.jsonl` | 테스트셋 (200건) | v0.2부터 JSONL로 전환 |
| `data/<version>/val.jsonl` | 검증셋 (200건) | v0.2부터 JSONL로 전환 |

## 7. 완료 기준 (Acceptance Criteria)

- [ ] `role_01~09_*.jsonl` 9개 파일 각각 정상 파싱(JSON 파싱 실패 행 0건), 각 200건(IT는 역할당 40건)
- [ ] `data.jsonl` 1,000건, 클래스당 정확히 200건, JSON 파싱 실패 행 0건
- [ ] `train.jsonl`(600)/`test.jsonl`(200)/`val.jsonl`(200) 클래스당 120/40/40건, JSON 파싱 실패 행 0건
- [ ] 5절의 비기능 요구사항(결측/중복/인코딩/개행/JSON 유효성/재현성) 전항목 통과
- [ ] 동일 입력으로 재실행 시 분할 결과 동일함을 확인(시드 고정 검증)

## 8. 리스크 및 참고사항

- `role_03_network.csv`의 이스케이프 오류는 v0.1(CSV 포맷)에서 발생한 사고이며, v0.2부터는
  JSONL 전환으로 이 유형의 파싱 사고 자체가 구조적으로 재발할 수 없다([[P1_Data_Preprocessing_Review]]
  3.1절). 다만 원본이 하류 결과에 의해 덮어써지지 않는다는 원칙은 포맷과 무관하게 동일하게
  적용된다 — `role_01~09_*.jsonl`가 원본(source of truth)이고, `data.jsonl`나 `test.jsonl`를
  직접 수정하면 다음 재조합/재분할 시 수정 내용이 사라진다 — [[CLAUDE.md]] 5절(입출력 보존
  원칙) 참고.
