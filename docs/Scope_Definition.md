...
- **기술 스택 및 프로세스**:
  ...
  - **임베딩 생성·저장·조회**: 기존 구축된 RAG API를 호출하여 처리 (직접 구현하지 않음 — 아래 2.1절 참고)
  - **분류 학습**: Scikit-learn Logistic Regression 모델 학습 및 추론

### 2.1 임베딩 인프라 — 기존 RAG API 활용

임베딩 모델 구동이나 벡터 저장소 관리를 직접 구현하지 않고, 이미 운영 중인 **RAG 서비스 API**(`localhost:28000`)를 호출하여 처리한다. 이 서비스는 내부적으로 **BGE-M3** 임베딩 모델과 **Qdrant** 벡터 DB를 사용한다.
...
입력 텍스트 (질의 + 응답)
  ...
  텍스트 전처리 (노이즈 제거)
      │
  [RAG API] POST /api/embeddings → 1024D 벡터
      │
  학습된 LR 모델 로드 (.pkl)
      │
...
| 임베딩 변환 및 캐싱 | RAG API 호출로 1024D 벡터 생성, MD5 해시 기반 중복 방지, Qdrant 적재 |
...
| **Phase 2** | **임베딩 변환** | RAG API 호출로 1024D 벡터 변환, MD5 캐싱, Qdrant 적재 | 임베딩 스크립트, 캐싱 로직 | `feature/phase2` |
